...
public class TimerUtils {
    ...
    public static NavyCountDownTimer getTimer(int style, Context mContext, long mGapTime, String mTimePattern, int mDrawableId){
        NavyCountDownTimer mCountDownTimer = null;
        switch (style){
            case JD_STYLE:
            ...
            case VIP_STYLE:
                ...
                break;
            case DEFAULT_STYLE:
                mCountDownTimer = new NavyCountDownTimer(mContext, mGapTime, mTimePattern, mDrawableId);
                break;
        }
    ...
    // 得到倒计时中字符串中的非数字的字符串，并把数字过滤掉重新组合成一个字符串，并把字符串拆分字符串数组，也就是保存倒计时中间的间隔
